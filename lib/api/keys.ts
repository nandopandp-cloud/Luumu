...
/** Resolve uma SDK key pública (pk_) para o projeto, se ativa. Atualiza lastUsedAt. */
export async function resolveKey(publicKey: string | null): Promise<ResolvedKey | null> {
  ...
  return { workspaceId: row.workspaceId, projectId: row.projectId, domains: (row.domains as string[]) ?? [] };
}
