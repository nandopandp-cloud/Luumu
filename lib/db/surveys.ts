...
/** Lista de pesquisas com métricas derivadas (nº respostas + score médio). */
export async function listSurveys() {
  const rows = await db
    .select()
    .from(surveys)
    .where(eq(surveys.workspaceId, WORKSPACE_ID))
...
export async function createSurveyFromTemplate(type: SurveyType) {
